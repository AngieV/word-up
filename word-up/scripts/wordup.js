...
function addNewWordSubmission(word) {
    ...
    // add "isRealWord" property
    // if the word is valid and hasn't already been used, add it
    if ((containsOnlyAllowedLetters(word) == true) && (alreadyUsed == false)) {
...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        // TODO_13(DONE) what should the url be?
        ...
        success: function(response) {
            ...
            //loop through array (wordSubmissions)
            //match word with correct index.word
            //set isRealWord on the object to the answer
              //example: model.wordSubmissions[index].isRealWord = //boolean
...
function render() {
    ...
    //$("#word-attempt-form").append(wordChips);
    ...
    if (gameOver) {
        ...
        $( "#textbox" ).attr("value", "").attr("disabled", true);
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
      ...
      // give the scoreChip appropriate css classes
         .attr("class", "tag tag-default tag-sm tag-primary");
...
function disallowedLetterChip(letter) {
    return $("<span></span>")
        .text(letter)
        .attr("class","tag tag-lg tag-danger disallowed-letter");
}
...
$(document).ready(function() {
    ...
    $("#textbox").on("input", function callback() {
...
function containsOnlyAllowedLetters(word) {
    // TODO_12(DONE)
    //return disallowedLettersInWord(word)==0;
...
function wordScore(word) {
    ...
    var letterScores = letters.map(function(letters){
      if (model.wordSubmissions.isRealWord == false) {
        return 0;
      } else {
        for ( var i = 0; i < letters.length; i++){
          return letterScore(letters[i]);
        }
      }
    });
